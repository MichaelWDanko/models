...
if [[ -f "$PID_FILE" ]]; then
  ...
  stop_pid "$pid"
  rm -f "$PID_FILE"
  echo "Stopped gemma-4-e4b-it server (PID ${pid:-unknown})"
  exit 0
fi
...
if [[ -n "$port_pid" ]]; then
  stop_pid "$port_pid"
  echo "Stopped gemma-4-e4b-it server listening on port $PORT (PID $port_pid)"
  exit 0
fi
...
if [[ -n "$pattern_pid" ]]; then
  ...
  echo "Stopped gemma-4-e4b-it server matching $MODEL_PATTERN (PID $pattern_pid)"
  exit 0
fi

echo "No running gemma-4-e4b-it server found"
